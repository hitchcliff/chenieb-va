let timer: any = null;
const edgeSize = window.innerWidth * 0.01;
...
export function drawGridLines() {
  ...
  for (let x = 0; x < incrementCount; x++) {
    // Draw the edges that delineate the scrolling zone.
    const edge = document.createElement('span');
    edge.style.position = 'fixed';
    edge.style.top = edgeSize + '%';
    edge.style.bottom = edgeSize + '%';
    edge.style.left = edgeSize + '%';
    edge.style.right = edgeSize + '%';
    edge.style.border = '2px solid #CC0000';
    edge.style.borderRadius = '5px 5px 5px 5px';

    const scene = document.getElementById('scrolling-screen');
...
export function handleMousemove(event: { clientX: any; clientY: any }) {
  ...
  (function checkForWindowScroll() {
    ...
    if (adjustWindowScroll()) {
      timer = setTimeout(checkForWindowScroll, 10);
    }
  })();
